// Deployed Google Apps Script URL for recording quiz results (Carry-all site).
// Update this when redeploying the Apps Script.
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwZnuZzlKisJliRR6LuP10uwgcPH6UBpDTnqKg0pNjrePIrglG7K08J1YaRl1VuzLI/exec';
...
function submitSupportQuiz(btn){
  ...
  showNamePrompt().then((name) => {
    if (!name) return;
    // Collect responses and compute score for support quizzes
    const responses = [];
    let correctCount = 0;
